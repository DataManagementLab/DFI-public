...
class IntegrationTestsAppend : public testing::Test 
{
...
public: 
  ...
  BufferWriterClient(string& bufferName, std::vector<DataType> *dataToWrite, int numThread = 4, FlowOptimization buffertype = FlowOptimization::BW) : 
  ...
  {
    //ARRANGE
    BufferWriterInterface *buffWriter = nullptr;
    RegistryClient regClient;
    if (buffertype == FlowOptimization::LAT)
    {
      buffWriter = new BufferWriter(bufferName, regClient, Config::DFI_SOURCE_SEGMENT_COUNT);
    }
    else
    {
      buffWriter = new BufferWriter(bufferName, regClient, Config::DFI_SOURCE_SEGMENT_COUNT);
