...
#include "Settings.h"
#include "DistributedReplicateJoin.h"
#include "utils/ThreadScheduler.h"

void usage(char* argv[]) {
	cout << "Usage: " << argv[0] << " nodeid total_nodes total_thread_count full_left_relation_size full_right_relation_size random_order(t/f) numa_node(0/1)" << endl;
}

int main(int argc, char *argv[])
{
    std::cout << "sizeof(CompressedTuple_t)="<< sizeof(CompressedTuple_t) << std::endl;
	if (argc <= 6) {
		usage(argv);
		return -1;
	}
    ...
    Settings::TOTAL_THREADS_COUNT = atoi(argv[3]);
    ...
    #ifdef USE_MULTICAST
    Settings::PART_THREADS_COUNT = Settings::TOTAL_THREADS_COUNT / 2;
    #else
    Settings::PART_THREADS_COUNT = Settings::TOTAL_THREADS_COUNT;
    #endif
    ...
    DistributedReplicateJoin join;

    utils::ThreadScheduler::start(Settings::TOTAL_THREADS_COUNT);
