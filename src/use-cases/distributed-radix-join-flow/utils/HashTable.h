...
{
    HashStd() {};
    HashStd(uint64_t mask) : _mask(mask)
    {
        // std::cout << "mask" << _mask << '\n';
    };
    ...
    {
// #ifdef SECOND_LOCAL_PARTITIONING
        // return hash_fn((key >> join::Config::RADIX_NETWORK_BITS) >> join::Config::RADIX_LOCAL_BITS) & _mask;
// #else
        // return hash_fn((key >> Settings::NETWORK_PARTITION_BITS) >> Settings::LOCAL_PARTITION_BITS) & _mask;
        return hash_fn(key);
// #endif
    }
...
class HashTable<uint64_t, ValueT, HashT, HashMapEqualTo<uint64_t>>
{
  ...
  public:
    ...
    bool find_bucket(uint64_t key, uint64_t &bucket)
    {
        ...
        std::cout << "longest chain " << get_longest_chain() << '\n';
        std::cout << "collisions " << get_num_collisions() << '\n';
        std::cout << "hashed value (first bucket): " << _hasher(key) << std::endl; 
        exit(EXIT_FAILURE);
        return false;
