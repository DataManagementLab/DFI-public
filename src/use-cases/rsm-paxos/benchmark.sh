...
MODE=1 # 0 = synchronous, 1 = asynchronous, 2 = batching
CMD_COUNT=1000000
...
# first REPLICA_COUNT entries are the replicas, the following are the clients
#REPLICA_IP_LIST="172.18.94.10 172.18.94.20 172.18.94.30 " 
REPLICA_IP_LIST="172.18.94.10 172.18.94.20 172.18.94.30 172.18.94.40 172.18.94.60 172.18.94.50 172.18.94.70 172.18.94.80 172.18.94.51 172.18.94.71 172.18.94.81"

BENCHNAME="${APPNAME}_r${REPLICA_COUNT}_c${CLIENT_COUNT}_full_1k_segs"
...
do
    ...
    do
        ...
        ssh $NODE "$APP_EXEC $BENCHFILE $MODE $CMD_COUNT $CMD_PER_SECOND $i $REPLICA_COUNT $CLIENT_COUNT $REPLICA_IP_LIST" | tee -a $LOG_DIR/node$i.log&
    done
    wait
