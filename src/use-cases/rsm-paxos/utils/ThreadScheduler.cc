...
const int utils::ThreadScheduler::cpus[] = {0, 1, 2, 3, 4, 5, 6, 7};
...
void utils::ThreadScheduler::doWork(int tid) {
	...
	std::vector<int> cpus = rdma::Config::NUMA_THREAD_CPUS[rdma::Config::RDMA_NUMAREGION];
	int cpu = cpus[tid];

	std::cout << "Starting thread on cpu: " << cpu << std::endl;
	cpu_set_t cpuset;
	CPU_ZERO(&cpuset);
	CPU_SET(cpu, &cpuset);
